...
# it proccesses everything into a new folder with csvs
#
class Rvd:
...
def to_csv(folder):
   for filename in os.listdir(folder):
    print(filename)
    # Check if the file ends with .vol extension
    if filename.endswith('.VOL'):
        # Create the new filename by replacing .vol with .csv
        new_filename = filename.replace('.VOL', '.csv')
...
folder = inputZipfile + '_converted'

#converts allthe data in the zip file

up = Rvd(inputZipfile+'.zip','Y')
up.startup()
...
if os.path.isfile(folder + '.zip'):
    ...
    print(f"File {folder + '.zip'} has been deleted.")
else:
    print(f"File {folder + '.zip'} not found.")

#converts the TOL to CSV
...
print('All .vol files have been renamed to .csv')
print(f'removing {inputZipfile}')
os.remove(inputZipfile+'.zip')